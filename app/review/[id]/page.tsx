...
export default function ReviewDetailPage() {
  ...
  const [moodColor, setMoodColor] = useState('sakura')
  const [published, setPublished] = useState(true)
  const [saving, setSaving] = useState(false)
  const [deleting, setDeleting] = useState(false)

  const review = useQuery(api.reviews.getById, { reviewId: id as Id<"reviews">, token: token ?? undefined })
  ...
  const moodColors: Record<string, string> = {
    ...
    gold: 'bg-gold-500',
    // legacy fallbacks
    inkPink: 'bg-sakura-500',
    inkLime: 'bg-mint-500',
    inkCyan: 'bg-sky-500',
    inkPurple: 'bg-kawaii-500',
    inkYellow: 'bg-gold-500'
  }
  ...
  if (!review) return <main className="py-10"><p>Not found. <Link href="/gallery" className="text-sakura-500">Back</Link></p></main>

  return (
    <main className="py-10">
      <Link href="/gallery" className="text-sm text-neutral-500 hover:text-sakura-500 mb-4 inline-block">&larr; Back</Link>
      <div className="max-w-3xl mx-auto mt-4">
        <div className={`h-2 rounded-t-xl ${moodColors[review.moodColor] || 'bg-sakura-500'}`} />
        <div className="rounded-b-xl border border-t-0 border-neutral-200 dark:border-neutral-800 p-6 sm:p-8">
          ...
          {editing ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2"><button onClick={handleSave} disabled={saving} className="rounded-md bg-sakura-500 text-white px-4 py-2 text-sm disabled:opacity-50">{saving ? 'Saving...' : 'Save'}</button><button onClick={() => setEditing(false)} className="rounded-md bg-neutral-200 dark:bg-neutral-700 px-4 py-2 text-sm">Cancel</button></div>
            </div>
          ) : (
            <>
          ...
          <div className="mt-8 pt-6 border-t border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {isOwner && !editing && <><button onClick={handleEdit} className="rounded-md bg-sky-500 text-white px-4 py-2 text-sm">Edit</button><button onClick={handleDelete} disabled={deleting} className="rounded-md bg-red-500 text-white px-4 py-2 text-sm disabled:opacity-50">{deleting ? '...' : 'Delete'}</button></>}
            </div>
          </div>
        </div>
      </div>
