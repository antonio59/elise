...
export default function ManagePage() {
  ...
  const artworks: any[] = useQuery(api.artworks.getAllArtworks) ?? []
  const reviews: any[] = useQuery(api.reviews.getAllReviews) ?? []
  const books: any[] = useQuery(api.books.getAllBooks) ?? []
  ...
  return (
    <main className="py-10">
      <div className="flex items-center justify-between mb-6">
        ...
        {message && (
          <span className="px-4 py-2 rounded-full bg-inkLime/20 text-inkLime text-sm font-medium">
            {message}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 p-4 bg-white dark:bg-neutral-900">
          ...
          <p className="text-3xl font-bold text-inkPurple">{books.length}</p>
          <p className="text-xs text-neutral-500 mt-1">
      ...
      <div className="flex gap-2 border-b border-neutral-200 dark:border-neutral-800 mb-6">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
            tab === 'artworks' ? 'border-inkLime text-inkLime' : 'border-transparent text-neutral-500'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
            tab === 'reviews' ? 'border-inkPink text-inkPink' : 'border-transparent text-neutral-500'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
            tab === 'books' ? 'border-inkPurple text-inkPurple' : 'border-transparent text-neutral-500'
          }`}
        >
          Books ({books.length})
        </button>
            ...
            artworks.map(art => (
              <div key={art._id} className="flex items-center gap-4 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link href={`/artwork/${art._id}`} className="px-3 py-1 rounded-md bg-inkCyan text-white text-sm">
                    View
                  </Link>
                  <button
            ...
            reviews.map(review => (
              <div key={review._id} className="flex items-center gap-4 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Link href={`/review/${review._id}`} className="px-3 py-1 rounded-md bg-inkCyan text-white text-sm">
                    View
                  </Link>
                  <button
          ...
          {books.length === 0 ? (
            ...
            books.map(book => (
              <div key={book._id} className="flex items-center gap-4 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    book.status === 'read' ? 'bg-inkCyan/20 text-inkCyan' : 
                    book.status === 'reading' ? 'bg-inkPink/20 text-inkPink' : 'bg-inkPurple/20 text-inkPurple'
                  }`}>
                  ...
                  <Link href={`/book/${book._id}`} className="px-3 py-1 rounded-md bg-inkCyan text-white text-sm">
                    View
                  </Link>
                  <button
