...
export default function BookForm({ book, onSaved }: BookFormProps) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await updateBook({ token, bookId: book._id, title, author, coverUrl: coverUrl || undefined, rating: status === 'read' ? rating : undefined, status, genre: genre || undefined, pagesTotal: pagesTotal || undefined, pagesRead: pagesRead || undefined, isFavorite })
        setMessage('Updated!')
      } else {
        await createBook({ token, title, author, coverUrl: coverUrl || undefined, rating: status === 'read' ? rating : undefined, status, genre: genre || undefined, pagesTotal: pagesTotal || undefined, pagesRead: pagesRead || undefined, isFavorite })
        setMessage('Added!')
        setTitle(''); setAuthor(''); setCoverUrl(''); setRating(0); setStatus('reading'); setGenre(''); setPagesTotal(0); setPagesRead(0); setIsFavorite(false)
      }
      onSaved?.()
