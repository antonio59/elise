...
export default function ReviewEditor() {
  ...
  const onPublish = async () => {
    ...
    try {
      ...
      setBookTitle(''); setAuthor(''); setRating(0); setMood('sakura'); setContent(''); setSelectedBookId(''); setImagePreview(null)
  ...
  const moodColorClasses: Record<string, string> = { sakura: 'bg-sakura-500', mint: 'bg-mint-500', sky: 'bg-sky-500', kawaii: 'bg-kawaii-500', gold: 'bg-gold-500' }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><span className="text-sm">Mood Color</span><div className="flex gap-2 mt-1">{moodOptions.map(color => (<button key={color} type="button" onClick={() => setMood(color)} className={`w-8 h-8 rounded-full ${moodColorClasses[color]} ${mood === color ? 'ring-2 ring-offset-2 ring-neutral-400 dark:ring-offset-neutral-900' : ''} transition-all`} />))}</div></div>
      </div>
