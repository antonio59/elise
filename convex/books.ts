...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    let coverUrl = sanitizeOptionalUrl(args.coverUrl) || sanitizePlainText(args.coverUrl, 500);
    const pagesTotal = args.pagesTotal !== undefined ? Math.max(0, args.pagesTotal) : undefined;
    let pagesRead = args.pagesRead !== undefined ? Math.max(0, args.pagesRead) : undefined;
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const updates: any = {};
    if (args.title !== undefined) updates.title = sanitizePlainText(args.title, 200);
    if (args.author !== undefined) updates.author = sanitizePlainText(args.author, 120);
    if (args.coverUrl !== undefined) updates.coverUrl = sanitizeOptionalUrl(args.coverUrl) || sanitizePlainText(args.coverUrl, 500);
    ...
    if (args.genre !== undefined) updates.genre = sanitizePlainText(args.genre, 80);
    if (args.series !== undefined) updates.series = sanitizePlainText(args.series, 120);
    if (args.published !== undefined) updates.published = args.published;
    if (args.pagesTotal !== undefined) updates.pagesTotal = Math.max(0, args.pagesTotal);
